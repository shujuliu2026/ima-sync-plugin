...
function classifyImaError (status, code, msg) {
  ...
  if (
    ...
    c === 429 ||
    c === 20002 ||
    /429|限频|过于频繁|rate.?limit|too many/i.test(text)
  ) {
    return 'rate'
  }
  if (
...
module.exports = { classifyImaError, buildImaHttpError, parseImaError, isSyncLimitError, isImaAuthError }
