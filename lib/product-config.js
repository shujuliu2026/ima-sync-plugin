...
function fromManifest (m) {
  ...
  const unbindSelfPath = String(m.license?.cloud?.unbindSelfPath || '/portal/unbind-self')
  ...
  return {
    ...
    qqGroup: String(m.brand?.qqGroup || '').trim(),
    ...
    licenseUnbindSelfUrl: `${licenseApiBase}${unbindSelfPath}`,
    licensePortalPath: String(m.license?.portalPath || '/p/ima-sync/license'),
    siteUrl
  }
}
