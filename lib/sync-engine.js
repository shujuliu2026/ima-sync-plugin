...
class ImaSyncEngine {
  ...
  async _pushNoteInner (file, force = false) {
    ...
    const importKey = frontmatter.import_key || title
    ...
    const attachResult = await syncAttachments(this.app, this.client, docId, body, file.path)
    if (attachResult.uploaded.length && attachResult.body !== body) {
