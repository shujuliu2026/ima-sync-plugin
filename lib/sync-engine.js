...
const { parseImaError, isSystemicBatchError } = require('./ima-errors')
...
const { t, resolveLang, localizeStatus, formatCodeList } = require('./i18n')
const { canUseTrust, trustDedupAllowed, trustVerifyAllowed, effectiveSyncFolders, canUseEnrich } = require('./license')
...
const { enrichNote } = require('./enrich-pipeline')
const { buildEnrichReport } = require('./enrich-report')
const { patchImaFrontmatter, buildLocalizedFrontmatterYaml } = require('./sync-frontmatter-i18n')
const {
  computeContentHash,
  parseNoteFile,
  isUnderSyncFolders,
...
class ImaSyncEngine {
  ...
  constructor (app, settings, onLog, control, onProgress, onFailedEntry) {
    ...
    this._formatItems = []
    /** @type {Array<{ path: string, status: string, source_url?: string, kind?: string, codes?: string[], preview?: string, error?: string }>} */
    this._enrichItems = []
    this._buildClient()
    this.running = false
  }
  ...
  initTrustSummary (summary) {
    ...
    summary.enriched = 0
    summary.enrich_degraded = 0
    summary.enrich_failed = 0
    summary.enrich_skipped = 0
    this._formatItems = []
    this._enrichItems = []
  ...
  finalizeFormatSummary (summary) {
    ...
    if (this._enrichItems?.length) {
      summary.enrichReport = buildEnrichReport(this._enrichItems)
    } else {
      summary.enrichReport = null
    }
    this._formatItems = []
    this._enrichItems = []
  ...
  async uploadGap () {
    ...
    const ms = Math.max(500, Number(this.settings.uploadGapMs) || 500)
  ...
  async handlePushError (summary, file, err) {
    ...
    const systemic = isSystemicBatchError(err)
    ...
    // 限频/超量/鉴权：中止整批，勿把后续笔记逐条标失败（否则会出现「失败≈全库」）
    if (!systemic) {
    ...
    if (limit || systemic) {
      summary.syncLimit = limit?.kind || 'auth'
      summary.stopped = true
      const warnKey = limit?.kind === 'quota'
        ? 'quotaExceeded'
        : (limit?.kind === 'auth' ? 'trustAuthFailed' : 'rateLimitExceeded')
      this.onLog(`⚠ ${this.tr(warnKey)}`)
      return true
    }
    return false
  }
  ...
  async handlePushErrorWithRetry (summary, file, err, retryFn) {
    ...
    // 限频耗尽后中止整批；不标失败，避免「失败≈全库」
    this.recordTrustItem(summary, file, { action: 'failed', error: this.tr('rateLimitExceeded') })
  ...
  async _pushNoteInner (file, force = false) {
    ...
    const enrichOnPush =
      canUseEnrich(this.settings) &&
      this.settings.enrich?.enabled !== false &&
      this.settings.enrich?.onPush === true
    if (enrichOnPush) {
      try {
        const enr = await enrichNote({
          path: file.path,
          body,
          frontmatter,
        ...
        const item = {
          ...
          kind: enr.kind || '',
          codes: enr.codes || [],
          preview: String(enr.fields?.body || enr.payloadMarkdown || '').slice(0, 80),
          error: enr.error || enr.skipReason || ''
        }
        this._enrichItems.push(item)
        if (this._activeSummary) {
          if (enr.status === 'enriched') this._activeSummary.enriched = (this._activeSummary.enriched || 0) + 1
          else if (enr.status === 'degraded') this._activeSummary.enrich_degraded = (this._activeSummary.enrich_degraded || 0) + 1
          else if (enr.status === 'failed') this._activeSummary.enrich_failed = (this._activeSummary.enrich_failed || 0) + 1
          else this._activeSummary.enrich_skipped = (this._activeSummary.enrich_skipped || 0) + 1
        }
        if ((enr.status === 'enriched' || enr.status === 'degraded') && enr.payloadMarkdown) {
          pushBody = enr.payloadMarkdown
          this.onLog(this.tr('enrichApplied', {
            path: file.path,
            status: localizeStatus(this.settings, enr.status)
          }))
        } else if (enr.status === 'failed') {
          this.onLog(this.tr('enrichFailedSoft', {
            path: file.path,
            code: formatCodeList(this.settings, [enr.error || (enr.codes || [])[0] || 'failed'].filter(Boolean))
          }))
        }
      } catch (err) {
        this._enrichItems.push({
          path: file.path,
          status: 'failed',
          codes: ['WEB_FETCH_FAILED'],
          error: String(err?.message || err || 'enrich_error')
        })
        if (this._activeSummary) this._activeSummary.enrich_failed = (this._activeSummary.enrich_failed || 0) + 1
        this.onLog(this.tr('enrichFailedSoft', {
          path: file.path,
          code: formatCodeList(this.settings, ['WEB_FETCH_FAILED'])
        }))
      }
    }
    ...
    const fmLang = resolveLang(this.settings)
    const syncAt = new Date().toISOString()
    await withTimeout(
      this.app.fileManager.processFrontMatter(file, (fm) => {
        // 排版只改正文推送；不把「上次推送/已应用规则」审计块写进笔记前置信息
        patchImaFrontmatter(fm, {
          syncIma: 'synced',
          syncError: '',
          syncAt,
          docId,
          contentHash,
          importKey: frontmatter.import_key || importKey
        }, fmLang)
      }),
      20000,
      'IMA_FM_TIMEOUT'
    ...
    if (this.trustEnabled()) {
      if (trustVerifyAllowed(this.settings) && shouldRunVerify(this.settings.trustCapabilities)) {
        ...
        await writeVerifyFrontmatter(this.app, file, verifyResult, this.settings)
  ...
  async pullNote (file, remote) {
    ...
    const fmBlock = buildLocalizedFrontmatterYaml(frontmatter, remote, contentHash, resolveLang(this.settings))
    const newRaw = `---\n${fmBlock}\n---\n\n${content}\n`
  ...
  async createNoteFromRemote (remote) {
    ...
    const fmInner = buildLocalizedFrontmatterYaml(
      { title: remote.title || safeName, import_key: importKey },
      remote,
      contentHash,
      lang
    )
  ...
  listFilesInFolder (folderPath) {
    const files = this.app.vault.getMarkdownFiles()
    const scope = effectiveSyncFolders(this.settings, this.settings.syncFolders)
    return files.filter((f) => {
      if (!isUnderSyncFolders(f.path, [folderPath || ''])) return false
      return isUnderSyncFolders(f.path, scope)
    })
  }
  ...
  async markConflict (file) {
    const lang = resolveLang(this.settings)
    await this.app.fileManager.processFrontMatter(file, (fm) => {
      patchImaFrontmatter(fm, { syncIma: 'conflict' }, lang)
    })
  }
}
