 ...
 * @param {'zh'|'en'} lang
 */
function buildFollowHint (wechat, qqGroup, lang) {
  const parts = []
  if (wechat) {
    parts.push(lang === 'en'
      ? `Follow WeChat: ${wechat} for latest version`
      : `关注公众号：${wechat}  获取最新版本`)
  }
  if (qqGroup) {
    parts.push(lang === 'en' ? `QQ group: ${qqGroup}` : `QQ 群：${qqGroup}`)
  }
  return parts.join(' · ')
}
...
function buildBrandOverrides (manifest) {
  ...
  const qqGroup = manifest.brand?.qqGroup || ''
  ...
  const followZh = buildFollowHint(wechat, qqGroup, 'zh')
  const followEn = buildFollowHint(wechat, qqGroup, 'en')
...
module.exports = { brandName, buildFollowHint, buildBrandOverrides, applyBrandStrings }
