...
const { patchImaFrontmatter } = require('./sync-frontmatter-i18n')
 ...
 * @param {object} [settings]
 */
async function writeVerifyFrontmatter (app, file, result, settings = {}) {
  const lang = resolveLang(settings)
  const detail = String(result.detail || '').trim()
  await app.fileManager.processFrontMatter(file, (fm) => {
    patchImaFrontmatter(fm, {
      syncVerify: result.status,
      // 精简写盘：核验结果进「同步.核验」；详情仅失败时保留；不写核验时间/查询
      verifyDetail: detail || undefined,
      clearVerify: !detail
    }, lang)
  })
}
