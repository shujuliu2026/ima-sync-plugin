...
const HOOKS = {
  ...
  /** 体验额度验签失败（篡改嫌疑 · 强制上报） */
  EXPERIENCE_TAMPER: 'ima.experience_tamper'
}
 ...
 */
function buildExperienceTamperEvent (ctx, detail = {}) {
  const claimed = detail.claimed && typeof detail.claimed === 'object' ? detail.claimed : {}
  return buildEvent({
    hook: HOOKS.EXPERIENCE_TAMPER,
    installId: ctx.installId,
    sessionId: ctx.sessionId,
    payload: {
      plugin_version: ctx.pluginVersion,
      reason: detail.reason === 'missing_signature' ? 'missing_signature' : 'bad_signature',
      claimed_enrich_parse_per_day: Number.isFinite(Number(claimed.enrich_parse_per_day))
        ? Math.floor(Number(claimed.enrich_parse_per_day))
        : null,
      claimed_batch_notes_per_day: Number.isFinite(Number(claimed.batch_notes_per_day))
        ? Math.floor(Number(claimed.batch_notes_per_day))
        : null,
      claimed_format_preview_per_day: Number.isFinite(Number(claimed.format_preview_per_day))
        ? Math.floor(Number(claimed.format_preview_per_day))
        : null
    }
  })
}
