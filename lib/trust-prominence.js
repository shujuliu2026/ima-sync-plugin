...
function noteVerifyBadge (fm) {
  const v = normalizeFrontmatter(fm)?.sync?.ima_verify
 ...
 * @param {number} [limit] 0 = 不截断（仍受硬顶 2000）
 */
function listVerifyFailedNotes (settings, app, limit = 8) {
  ...
  const files = folders.length
    ...
    : all
  const hardCap = limit > 0 ? limit : 2000
  /** @type {{ path: string, detail: string }[]} */
  const out = []
  for (const f of files) {
    const fm = normalizeFrontmatter(app.metadataCache.getFileCache(f)?.frontmatter)
    if (fm?.sync?.ima_verify !== 'failed') continue
    out.push({
      path: f.path,
      detail: String(fm.ima_verify_detail || fm.ima_verify_query || '').slice(0, 120)
    })
 ...
 * @param {import('obsidian').App} app
 */
function countVerifyFailedNotes (settings, app) {
  return listVerifyFailedNotes(settings, app, 0).length
}
